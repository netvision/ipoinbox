  ...
  <q-page class="q-pa-md">
    ...
    <q-card flat class="rounded-border bg-orange-2">
      ...
      <div class="q-pa-sm flex q-gutter-sm bg-orange-3 rounded-borders">
        <span><a :href="ipo.company_url" target="_blank">{{ipo.company_url}}</a></span> <q-separator color="orange" vertical /> <a v-if="nse && nse.url" :href="nse.url">NSE</a> <q-separator color="orange" vertical /> <a v-if="bse && bse.url" :href="bse.url">BSE</a><q-separator color="orange" vertical /><span> Registrar: <a :href="registrar.url">{{registrar.name}}</a></span> 
      </div>
      <div class="row q-col-gutter-sm q-pt-md justify-between item-stretch" v-if="new Date(ipo.close_date).getTime() < Date.now()">
        ...
        <div class="col-12 col-md-4 q-ma-sm bg-orange-1" v-if="nse">
          <q-item>
            <q-item-section>
        ...
        <div class="col-12 col-md-grow q-ma-sm bg-orange-1">
          <q-item>
          <q-item-section>
           <vue-slider :min="+eod.Low" :max="+eod.High" v-model="eod.Close" :marks="[eod.Low, eod.High]" :tooltip="'always'" />
          </q-item-section>
          </q-item>
          <q-item>
            <q-item-section>
...
import 'vue-slider-component/theme/antd.css'
  ...
  onMounted(async() => {
    ...
    let scrips = await axios.get('https://droplet.netserve.in/nse/index').then(r => JSON.parse(r.data))
    let niftyEod = await axios.get('https://droplet.netserve.in/nse/eod').then(r => JSON.parse(r.data))
    ...
    let scrip = scrips.data.filter(s => s.NSESymbol === nse.value.scrip_code)
    if(scrip.length > 0) {
      curInfo.value = scrip[0]
    ...
    val.value = [nse.value.low, nse.value.high]
    useMeta({
