...
<q-btn class="fixed_right" @click="nextIpo(nxt)" icon="navigate_next" />
<q-btn class="fixed_left tw-hover:scale-110" @click="nextIpo(pre)" icon="navigate_before" />
  <q-page class="q-pa-md">
    ...
    <q-card flat class="rounded-border bg-orange-2">
      ...
      <q-card-section v-if="nse?.listing_date || bse?.listing_date">
...
<script setup>
  ...
  const init = async(ipo_id) => {
    ...
    if(listings.length > 0){
        ...
        if (bse.value && ipo.value.issue_price) bse.value.issue_price = ipo.value.issue_price
        ...
        if(nse.value && nse.value.scrip_code && nse.value.listing_date){
            ...
            nse.value.info = scrips.value.data.filter(s => s.NSESymbol === nse.value.scrip_code)[0]
            nse.value.eod = nseEod.value.data.filter(e => e.co_code === nse.value.info.co_code)[0]

            let nselive = await axios.get('https://stockapi.ipoinbox.com/quote?companyName='+nse.value.scrip_code).then(r => r.data)
            console.log(nselive)
            nse.value.liveQuote = nselive.data[0]
            nse.value.liveQuote.lastUpdate = nselive.lastUpdateTime
        }
        console.log(nse.value)      
    }
